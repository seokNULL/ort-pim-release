...
TEST_P(SessionStateAddGetKernelTest, AddGetKernelTest) {
  ...
  ASSERT_STATUS_OK(s.FinalizeSessionState(ORT_TSTR(""), kernel_registry_manager, align_map));
...
TEST_P(SessionStateTestP, TestInitializerProcessing) {
  ...
  ASSERT_TRUE(status.IsOK()) << status;
  std::map<std::string, std::vector<std::string>> align_map = {};
  ASSERT_STATUS_OK(session_state.FinalizeSessionState(oss.str(), krm, align_map));
