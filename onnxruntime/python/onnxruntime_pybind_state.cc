...
// pim
#ifdef USE_PIM
onnxruntime::ArenaExtendStrategy pim_arena_extend_strategy = onnxruntime::ArenaExtendStrategy::kSameAsRequested;
#include "core/providers/pim/pim_allocator.h"
OrtDevice::DeviceId pim_device_id = 0;
uint64_t pim_mem_limit = std::numeric_limits<size_t>::max();
#endif
...
static const std::vector<std::string>& GetAllProviders() {
  static std::vector<std::string> all_providers = {kTensorrtExecutionProvider, kCudaExecutionProvider,
                                                   ...
                                                   kDmlExecutionProvider, kPimExecutionProvider,kCpuExecutionProvider};
...
static bool IsPimDeviceIdValid(const onnxruntime::logging::Logger& logger, int id) {
  return true;
}

static AllocatorPtr GetPimAllocator(OrtDevice::DeviceId id) {
  static std::unordered_map<OrtDevice::DeviceId, AllocatorPtr> id_to_allocator_map;
  ...
  if (id_to_allocator_map.find(id) == id_to_allocator_map.end()) {
    ...
    AllocatorCreationInfo default_memory_info(
      ...
      true,
      {pim_mem_limit,
       static_cast<int>(pim_arena_extend_strategy),
       -1, -1});
    auto allocator = CreateAllocator(default_memory_info);
...
static void CpuToPimMemCpy(void* dst, const void* src, size_t num_bytes) {
...
#endif
    } else if (type == kPimExecutionProvider) {
#ifdef USE_PIM
      const PIMExecutionProviderInfo info =
          [&]() {
                  PIMExecutionProviderInfo info{};
                  info.device_id = pim_device_id;
                  info.hip_mem_limit = pim_mem_limit;
                  info.arena_extend_strategy = pim_arena_extend_strategy;
                  return info;
                }();
      RegisterExecutionProvider(
          sess, *onnxruntime::CreateExecutionProviderFactory_PIM(info));
#endif
...
void InitializeSession(InferenceSession* sess, const std::vector<std::string>& provider_types,
                       const ProviderOptionsVector& provider_options, std::map<std::string, std::vector<int>> partition_map, std::map<std::string, std::vector<std::string>> align_map) {
  ProviderOptionsMap provider_options_map;
  GenerateProviderOptionsMap(provider_types, provider_options, provider_options_map);
  sess->partition_map_ = partition_map;
  sess->align_map_ = align_map;
...
#endif
#ifdef USE_PIM
            onnxruntime::CreateExecutionProviderFactory_PIM(
                [&]() {
                  PIMExecutionProviderInfo info{};
                  info.device_id = pim_device_id;
                  info.hip_mem_limit = pim_mem_limit;
...
#endif
        } else if (GetDeviceName(device) == PIM) {
#ifdef USE_PIM
          if (!IsPimDeviceIdValid(logging::LoggingManager::DefaultLogger(), device.Id())) {
            throw std::runtime_error("The provided device id doesn't match any available PIM on the machine.");
          }

          CreateGenericMLValue(nullptr, GetPimAllocator(device.Id()), "", array_on_cpu, ml_value.get(),     true, false, CpuToPimMemCpy);

#else
      throw std::runtime_error(
          "Can't allocate memory on the PIM device using this package of OnnxRuntime. ");
#endif
        } else {
  ...
  py::class_<PyInferenceSession>(m, "InferenceSession", R"pbdoc(This is the main class used to run a model.)pbdoc")
      ...
      .def(
          ...
          [](PyInferenceSession* sess,
             ...
             std::map<std::string, std::vector<int>> partition_map = {},
             std::map<std::string, std::vector<std::string>> align_map = {}) {
            InitializeSession(sess->GetSessionHandle(), provider_types, provider_options, partition_map, align_map);
          },
      ...
      .def("set_partitions", 
        [](PyInferenceSession* sess, std::map<std::string, std::vector<int>> partition_map) {
          OrtPybindThrowIfError(sess->GetSessionHandle()->SetPartitions(partition_map));
      })
