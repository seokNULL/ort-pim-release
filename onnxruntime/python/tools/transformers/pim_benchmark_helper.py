...
def create_onnxruntime_session(device,
                               onnx_model_path,
                               use_pim,
                               partition_map=None,
                               align_map=None,
    ...
    try:
        ...
        elif (not use_pim) and (version.parse(onnxruntime_version) < version.parse('1.3.0')):
        ...
        execution_providers = ['CPUExecutionProvider'
                               ] if not use_pim else ['PIMExecutionProvider', 'CPUExecutionProvider']
        sess_options.enable_mem_pattern = False
        sess_options.enable_cpu_mem_arena = False      
        if partition_map:
            sess_options.enable_partitioning = True

        sess_options.profile_file_prefix = device + "_"  

        session = InferenceSession(onnx_model_path, sess_options=sess_options, providers=execution_providers, partition_map=partition_map, align_map=align_map)
        # session = InferenceSession(onnx_model_path, sess_options=sess_options, providers=execution_providers, align_list=align_list, align_map=align_map, memcpy_list=memcpy_list)

        # session.set_partitions(partition_map=partition_map, align_list=align_list, align_map=align_map, memcpy_list=memcpy_list)
    except:
...
def prepare_environment(cache_dir, output_dir, use_pim):
    ...
    import onnxruntime
    if use_pim:
        assert 'PIMExecutionProvider' in onnxruntime.get_available_providers(
        ), "Check PIM device."
