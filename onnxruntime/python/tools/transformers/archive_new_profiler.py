...
def generate_dcg(cpu_profile_file, pim_profile_file):
    ...
    # Check DA edges in edge_dict

    tot_ua_edge_list = []
    for key, value in edge_dict.items():
        dma_size = value[0][2]
        ua_edge = str(value[0][2]) + str(value[0][0]) + str(value[0][1])
        print("KEY: ", key)
        if ua_edge not in tot_ua_edge_list:
            tot_ua_edge_list.append(ua_edge)

    print("tot_ua_edge_list")
    print(tot_ua_edge_list)

    # Check DA edges in pim profile
    ua_edge_list = {}
    with open(pim_prof, 'r') as f:
        data = json.load(f)
        for item in data:
    ...
    print("Current ua edge list")
    print(ua_edge_list.keys())
    print("\n")
    print(ua_edge_list)
...
def run(args):
    ...
    print("GENERATE DCG")
    DCG = generate_dcg(cpu_profile_file, pim_profile_file)
