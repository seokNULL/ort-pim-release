...
common::Status InferenceSession::TransformGraph(onnxruntime::Graph& graph,
  ...
  if (session_options_.enable_partitioning) {
    mode = GraphPartitioner::Mode::kPartition;
    partitioner.partition_map_ = partition_map_;
  }
...
common::Status InferenceSession::SetPartitions(std::map<std::string, std::vector<int>>& partition_map) {
