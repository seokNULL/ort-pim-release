...
static common::Status DeserializeTensorProto(const Env& env, const std::basic_string<PATH_CHAR_TYPE>& proto_path,
  ...
  bool is_pim = strcmp(alloc_info.name, PIM) == 0 ? 1 : 0;

  if (!is_pim && m.GetLen() < cpu_tensor_length) {
    return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL, "Internal error. The preallocated buffer is too small. Requires ",
                           cpu_tensor_length, ", Got ", m.GetLen());
  }
  ...
  if (!is_pim) {
    ...
    std::vector<int64_t> tensor_shape = p_deserialize_tensor.Shape().GetDims();
    std::vector<int64_t> org_tensor_shape = tensor_shape;
    MLDataType bf16_type = DataTypeImpl::GetType<uint16_t>();

    if (ort_value.need_align) {
      size_t dims = tensor_shape.size();
      if (dims != 0) {
        size_t last_dim = tensor_shape[dims - 1];
        if (last_dim % 512 != 0) {
          size_t N_pad = 512 * (last_dim / 512 + 1);
          tensor_shape.at(dims - 1) = N_pad;
          layout_changed = true;
        }
      }    
    }
    ...
    //   }
    // }   
  if(ort_value.is_decoupled_weight) {
    TensorShape _tensor_shape{org_tensor_shape};
    p_tensor = onnxruntime::make_unique<Tensor>(bf16_type, _tensor_shape, m.GetBuffer(),
                                                m.GetAllocInfo());    
    p_tensor->SetOriginalShape(org_tensor_shape);
    p_tensor->SetIsPim();    
    p_tensor->SetIsMatrixBDecooupled();
  }
  else{
    TensorShape _tensor_shape{tensor_shape};
    p_tensor = onnxruntime::make_unique<Tensor>(bf16_type, _tensor_shape, m.GetBuffer(),
                                                m.GetAllocInfo());    
    p_tensor->SetOriginalShape(org_tensor_shape);
    p_tensor->SetIsPim();
    if (layout_changed) {p_tensor->SetLayoutChanged();}
    // if (ort_value.need_align) {p_tensor->SetIsMatrixB();}
    if (ort_value.is_mm_b) {p_tensor->SetIsMatrixB();}
  }
...
common::Status SaveInitializedTensors(
  ...
  for (const auto& entry : id_to_initialized_tensor) {
    ...
    // Align
    std::string s(name);
    // std::map<std::string, std::vector<std::string>>::iterator it = align_map.begin();
    for (auto& it: align_map) {
      if (it.first == "mm_b") {
        if (std::find(it.second.begin(), it.second.end(), s) != it.second.end())
          {
            ort_value.need_align = true;
            ort_value.is_mm_b = true;
          }
      } else {
        if (std::find(it.second.begin(), it.second.end(), s) != it.second.end())
          {
      ...
      if (it.first == "decoupled_registry"){
        if (std::find(it.second.begin(), it.second.end(), s) != it.second.end())
          ort_value.is_decoupled_weight = true;          
      }
    }
    ...
    if (user_supplied_initializer_ids.find(entry.first) != user_supplied_initializer_ids.end()) {
      ...
      ORT_RETURN_IF_ERROR(planner.GetPreallocatedBuffer(ort_value_index, name, m, align_map));
#ifndef NDEBUG
