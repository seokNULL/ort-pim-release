...
class SimpleTensorAllocator : public ITensorAllocator {
 ...
 public:
  ...
  common::Status GetPreallocatedBuffer(int ort_value_index, const char* name, std::unique_ptr<MemBuffer>& out, std::map<std::string, std::vector<std::string>>& align_map) override;
