...
static common::Status AllocateHelper(const AllocatorPtr& allocator,
                                     ...
                                     bool is_pim, bool input_copy, MLDataType dtype, bool need_align, bool is_mm_b) {
  if (!allocator) {
  ...
  if (!is_pim) {
    ...
    if (input_copy) {
      p_tensor = onnxruntime::make_unique<Tensor>(fetched_tensor.DataType(),
                                                  fetched_tensor.Shape(),
                                                  allocator);
    } else {
        // p_tensor = onnxruntime::make_unique<Tensor>(dtype,
        //                                             // fetched_tensor.GetOriginalShape(),
        //                                             fetched_tensor.Shape(),
        //                                             allocator);    
      if (fetched_tensor.GetLayoutChanged()) {
        p_tensor = onnxruntime::make_unique<Tensor>(dtype,
                                                    fetched_tensor.GetOriginalShape(),
                                                    // fetched_tensor.Shape(),
                                                    allocator);      
    ...
    std::vector<int64_t> tensor_shape = fetched_tensor.Shape().GetDims();
    std::vector<int64_t> org_tensor_shape = tensor_shape;
    bool layout_changed = false;
    if (need_align) {
      int64_t dims = tensor_shape.size();
      int64_t last_dim = tensor_shape[dims - 1];

      if (last_dim % 512 != 0) {
        tensor_shape.at(dims - 1) = 512 * (last_dim / 512 + 1);
        layout_changed = true;
      }
    }
    ...
    // }
    // //
    auto bf16_type = DataTypeImpl::GetType<uint16_t>();
    std::unique_ptr<Tensor> p_tensor = onnxruntime::make_unique<Tensor>(bf16_type,
                                                                        tensor_shape,
                                                                        allocator);
    p_tensor->SetIsPim();
    if (layout_changed) {p_tensor->SetLayoutChanged();}
    // if (need_align) {p_tensor->SetIsMatrixB();}
    if (is_mm_b) {p_tensor->SetIsMatrixB();}

    // p_tensor->SetOriginalShape(fetched_tensor.Shape());
    p_tensor->SetOriginalShape(org_tensor_shape);
...
static Status BatchOrCopyMLValue(const SessionState& session_state,
  ...
  bool is_pim = copy_info.target_device.Type() == OrtDevice::PIM ? true : false;
//
  auto& source_tensor = source_mlvalue.Get<Tensor>();

  // if (source_mlvalue.need_align) {source_tensor.SetNeedAlign();}
  bool need_align = source_mlvalue.need_align;
  bool is_mm_b = source_mlvalue.is_mm_b;

  MLDataType dtype;
  if (input_copy) {
    session_state.SetOriginalType(source_tensor.DataType());
    dtype = source_tensor.DataType();
  } else {
    // dtype = session_state.GetOriginalType();
    // temporal approach
    dtype = DataTypeImpl::GetType<float>();
  }
  ...
  if (!target_mlvalue.IsAllocated()) {
    ...
    ORT_RETURN_IF_ERROR(utils::AllocateHelper(allocator, source_tensor, target_mlvalue, is_pim, input_copy, dtype, need_align, is_mm_b));
  }
