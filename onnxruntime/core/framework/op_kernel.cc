...
Tensor* OpKernelContext::Output(int index, const TensorShape& shape, bool pim_output) {
  auto p_ml_value = OutputMLValue(index, shape, 0, pim_output);
...
OrtValue* OpKernelContext::OutputMLValue(int index, const TensorShape& shape, size_t nnz, bool pim_output) {
  if (index < 0 || index >= OutputCount())
    return nullptr;
  ...
  OrtValue* p_ml_value = nullptr;
  Status status = execution_frame_->GetOrCreateNodeOutputMLValue(GetOutputArgIndex(index), &shape, p_ml_value, nnz, pim_output);
  ORT_ENFORCE(status.IsOK(), status.ErrorMessage());
  return p_ml_value;
}
