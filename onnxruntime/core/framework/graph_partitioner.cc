...
static Status PartitionOnnxFormatModelImpl(Graph& graph, bool export_dll, FuncManager& func_mgr,
  ...
  if (mode == GraphPartitioner::Mode::kPartition) {
    ...
    graph_viewer.SetPartitionVector(partition_nodes);
  } 

  // std::cout << "DEBUG PARTITONER" << std::endl;
  // for (auto i : partition_nodes) {
...
Status GraphPartitioner::Partition(Graph& graph, bool export_dll, FuncManager& func_mgr, Mode mode,
  ...
  if (mode == Mode::kNormal || mode == Mode::kAssignOnly || mode == Mode::kPartition) {
#if !defined(ORT_MINIMAL_BUILD)
