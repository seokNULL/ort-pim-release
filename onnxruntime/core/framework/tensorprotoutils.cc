...
template <size_t alignment>
common::Status GetSizeInBytesFromTensorProtoToPim(const ONNX_NAMESPACE::TensorProto& tensor_proto, size_t* out, std::vector<size_t> &tensor_shape) {
  const auto& dims = tensor_proto.dims();
  size_t size = 1;
  for (google::protobuf::int64 dim : dims) {
...
template common::Status GetSizeInBytesFromTensorProto<kAllocAlignment>(const ONNX_NAMESPACE::TensorProto& tensor_proto,
                                                                       size_t* out);
template common::Status GetSizeInBytesFromTensorProtoToPim<kAllocAlignment>(const ONNX_NAMESPACE::TensorProto& tensor_proto,
                                                                       size_t* out, std::vector<size_t> &tensor_shape);
