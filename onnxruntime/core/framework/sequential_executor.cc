  ...
  for (auto i = 0; i < op_kernel_context->OutputCount(); i++) {
    ...
    if (p_output != nullptr && p_output->IsTensor()) {
      ...
      std::cout << node_name << " output[" << i << "]"
                ...
                << " original_shape=" << tensor.GetOriginalShape().ToString()
  ...
  for (auto i = 0; i < input_count; i++) {
    ...
    if (p_input != nullptr && p_input->IsTensor()) {
      ...
      std::cout << node_name << " input[" << i << "]"
                ...
                << " size=" << tensor_size
                << " shape=" << tensor_shape.ToString()
                << " original_shape=" << p_tensor->GetOriginalShape().ToString()
...
// Enable TRACE_EXECUTION compile flag to dump execution plan
  ...
  // std::vector<std::string> output_need_align = {};
  std::string ep_type;
...
// Profile input, output of the node
// start
    std::vector<std::string> input_nodes;
    std::vector<std::string> output_nodes;

    for (auto it = node.InputNodesBegin(); it != node.InputNodesEnd(); ++it) {
      input_nodes.push_back((*it).Name());
    }
    for (auto it = node.OutputNodesBegin(); it != node.OutputNodesEnd(); ++it) {
      output_nodes.push_back((*it).Name());
      // if (node.GetOutputNeedAlign()) {
      //   output_need_align.push_back((*it).Name());
      // }
    }
    std::string input_nodes_str;
    std::string output_nodes_str;
    for (const auto &piece : input_nodes) input_nodes_str += piece + " ";
    for (const auto &piece : output_nodes) output_nodes_str += piece + " ";
// end
    ...
    Status compute_status;
    {
      ...
      auto _ep_type = p_op_kernel->Node().GetExecutionProviderType();
      ep_type = _ep_type;
      auto op_type = p_op_kernel->Node().OpType();

      if (op_type == "MemcpyFromHost") {
        op_kernel_context.memcpy_cpu_to_pim = true;
    ...
    if (is_profiler_enabled) {
      ...
      session_state.Profiler().EndTimeAndRecordEvent(profiling::NODE_EVENT,
                                                     ...
                                                     kernel_begin_time,
                                                     op_kernel_context.tensor_alloc_dur,
                                                     op_kernel_context.dma_tx,
                                                     ep_type,
                                                     // Log additional operation args / info.
                                                     {
                                                         ...
                                                         {"input_nodes", input_nodes_str},
                                                         {"output_nodes", output_nodes_str}
                                                     });
