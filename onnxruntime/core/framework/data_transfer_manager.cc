...
Status DataTransferManager::CopyTensor(const Tensor& src, Tensor& dst, int exec_queue_id) const {
  // Due to data alignment of PIM tensor
  // if (!(src.Location().device.Type() == OrtDevice::PIM || dst.Location().device.Type() == OrtDevice::PIM)) {
  //   if (src.Shape().Size() != dst.Shape().Size()) {
  //     return Status(ONNXRUNTIME, FAIL, "Tensor size mismatch");
  //   }
  // }
  ...
  if (!((src.GetIsMatrixB() && src.Location().device.Type() == OrtDevice::PIM) || (dst.GetIsMatrixB() && dst.Location().device.Type() == OrtDevice::PIM))) {
    if (!dst.GetLayoutChanged() && !src.GetLayoutChanged()) {
      if (src.Shape().Size() != dst.Shape().Size()) {
        return Status(ONNXRUNTIME, FAIL, "Tensor size mismatch");
      }
    }
...
Status DataTransferManager::CopyTensor(const Tensor& src, Tensor& dst, OpKernelContext& ctx, int exec_queue_id) const {
  // Due to data alignment of PIM tensor
  // if (!(src.Location().device.Type() == OrtDevice::PIM || dst.Location().device.Type() == OrtDevice::PIM)) {
  //   if (src.Shape().Size() != dst.Shape().Size()) {
  //     return Status(ONNXRUNTIME, FAIL, "Tensor size mismatch");
  ...
  for (auto& data_transfer : datatransfers_) {
    ...
    return data_transfer->CopyTensor(src, dst, ctx, exec_queue_id);
  }
