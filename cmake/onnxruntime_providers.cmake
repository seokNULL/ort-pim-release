...
if(onnxruntime_USE_PIM)
  set(PROVIDERS_PIM onnxruntime_providers_pim)
  list(APPEND ONNXRUNTIME_PROVIDER_NAMES pim)
endif()
...
if (onnxruntime_USE_PIM)
  set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -Wno-unused-variable -Wno-unused-parameter")
  add_definitions(-DUSE_PIM=1)
  include_directories(${PROJECT_SOURCE_DIR}/external/protobuf)
  include_directories(${ONNXRUNTIME_ROOT}/../cmake/external/onnx ${eigen_INCLUDE_DIRS})
  file(GLOB_RECURSE
    onnxruntime_providers_pim_cc_srcs CONFIGURE_DEPENDS
    "${ONNXRUNTIME_ROOT}/core/providers/pim/*.h"
    "${ONNXRUNTIME_ROOT}/core/providers/pim/*.cc"
  )
  source_group(TREE ${ONNXRUNTIME_ROOT}/core FILES ${onnxruntime_providers_pim_cc_srcs})
  add_library(onnxruntime_providers_pim ${onnxruntime_providers_pim_cc_srcs})

  onnxruntime_add_include_to_target(onnxruntime_providers_pim onnxruntime_common onnxruntime_framework onnx onnx_proto protobuf::libprotobuf-lite flatbuffers)
  add_dependencies(onnxruntime_providers_pim onnx ${onnxruntime_EXTERNAL_DEPENDENCIES})

  message("-- Find PIM library: ${CMAKE_OSX_ARCHITECTURES}")
  message("   -- Target Platform: ${CMAKE_OSX_ARCHITECTURES}")
  if (CMAKE_OSX_ARCHITECTURES STREQUAL "arm64" OR CMAKE_OSX_ARCHITECTURES STREQUAL "arm")
    target_include_directories(onnxruntime_providers_pim PUBLIC ${ONNXRUNTIME_ROOT} ${onnxruntime_PIM_SW_HOME}/include)
    find_library(PIM_MEM NAMES libpim_mem.so HINTS ${onnxruntime_PIM_SW_HOME}/lib REQUIRED)
    message("   -- Linking PIM memory library: ${PIM_MEM}")
    target_link_libraries(onnxruntime_providers_pim ${PIM_MEM})
  elseif (CMAKE_OSX_ARCHITECTURES STREQUAL "x86_64" OR CMAKE_OSX_ARCHITECTURES STREQUAL "x64")
    find_library(PIM_MEM NAMES libpim_mem.so REQUIRED)
    target_link_libraries(onnxruntime_providers_pim ${PIM_MEM})
    message("   -- Linking PIM memory library: ${PIM_MEM}")
  endif()
endif()
