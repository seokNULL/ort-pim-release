...
class OpKernelContext {
  ...
  */
  ...
  bool memcpy_cpu_to_pim = false;
  bool memcpy_pim_to_cpu = false;
  ...
  Tensor* Output(int index, const TensorShape& shape, bool output = false);
 ...
 protected:
  ...
  OrtValue* OutputMLValue(int index, const TensorShape& shape, size_t nnz = 0, bool pim_output = false);
 private:
